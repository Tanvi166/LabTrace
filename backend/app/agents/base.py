...
class BaseAgent(Generic[I, O], ABC):
    def __init__(self, name: str, description: str, input_schema: Type[I], output_schema: Type[O]):
        self.name = name
    ...
    def run(self, input_data: I, analysis_run_id: str, db: Session) -> O:
        log_entry = AgentLog(
        ...
        try:
            ...
            log_entry.structured_output = {
                "input_summary": {
                    ...
                    "fields": list(input_data.model_dump().keys()),
                },
                "output": output.model_dump(),
            }
            log_entry.summary = getattr(output, 'summary', f"{self.name} completed successfully.")
            db.commit()
